import os
import numpy as np
import matplotlib.pyplot as plt
from PIL import Image

# Constants
eleH = 10  # Number of elements in height
eleL = 8  # Number of elements in length
steps = 1000
dt = 20
scaling_factor = 0.1
crack_factor = 4


def load_data(file_path):
    return np.loadtxt(file_path, delimiter=None)


def process_data(step):
    print(f"Processing step {step}")

    C1 = [[None for _ in range(eleL)] for _ in range(eleH)]
    s1 = [[None for _ in range(eleL)] for _ in range(eleH)]
    s2 = [[None for _ in range(eleL)] for _ in range(eleH)]

    for i in range(eleH):
        for j in range(eleL):
            cracking_angles = load_data(f'plot/MVLEM_cracking_angle_ele_{i + 1}_panel_{j + 1}.txt')
            interlock1 = load_data(f'plot/MVLEM_strain_stress_inter1_ele_{i + 1}_panel_{j + 1}.txt')
            interlock2 = load_data(f'plot/MVLEM_strain_stress_inter2_ele_{i + 1}_panel_{j + 1}.txt')

            C1[i][j] = cracking_angles[step, 1:]  # Assuming first column is time
            s1[i][j] = interlock1[step, 1:]  # Assuming first column is time
            s2[i][j] = interlock2[step, 1:]  # Assuming first column is time

    # Load displacement data
    dx = load_data('plot/MVLEM_Dx.txt')[step, 1:eleH + 2]  # +2 to include both ends
    print(dx)
    return C1, s1, s2, dx


def plot_cracks(step):
    C1, s1, s2, dx = process_data(step)
    plt.figure(figsize=(5, 8))

    x1 = np.arange(eleL + 1)
    y1 = np.arange(eleH + 1)

    # Plot horizontal and vertical lines with displacement
    for j in range(eleH):
        hor_X = [x + (-dx[j]) * scaling_factor for x in x1]
        hor_y = [y1[j]] * len(x1)
        plt.plot(hor_X, hor_y, 'k', linewidth=0.2)

    for i in range(eleL):  # Correction needed here
        ver_x = [x1[i] + (-dx[j]) * scaling_factor for j in range(eleH)]  # This creates a list of length eleH
        ver_y = y1[:eleH]  # This creates a list of length eleH + 1
        plt.plot(ver_x, ver_y, 'k', linewidth=0.2)

    # Calculate center points with displacement
    x0 = [[x1[i] + (-0.5 * dx[j] - 0.5 * dx[j]) * scaling_factor for i in range(eleL)] for j in range(eleH)]
    y0 = [[y1[j] + 0.5 for i in range(eleL)] for j in range(eleH)]

    # Plot cracks
    for j in range(eleL):
        for i in range(eleH):
            theta1 = C1[i][j][0]  # Assuming first value is the angle
            if theta1 == 10:
                continue

            x_start = x1[j] + (-0.5 * dx[i] - 0.5 * dx[i]) * scaling_factor
            x_end = x1[j + 1] + (-0.5 * dx[i] - 0.5 * dx[i]) * scaling_factor
            x = np.linspace(x_start, x_end, 100)

            k = np.tan(np.radians(theta1))  # Convert angle to radians
            b = y0[i][j] - k * x0[i][j]
            y = k * x + b

            mask = (y >= y1[i]) & (y <= y1[i + 1])
            x, y = x[mask], y[mask]

            if len(x) == 0:
                continue

            strain = max(s1[i][j][0], s2[i][j][0])  # Assuming first value is the strain
            if strain < 5.0e-5 * crack_factor:
                continue
            elif strain < 1.0e-4 * crack_factor:
                linewidth = 0.15
            elif strain < 3.0e-4 * crack_factor:
                linewidth = 0.45
            elif strain < 1e-3 * crack_factor:
                linewidth = 1.5
            elif strain < 2.0e-3 * crack_factor:
                linewidth = 3
            else:
                linewidth = 4

            plt.plot(x, y, 'r', linewidth=linewidth)

    plt.axis([0-2, eleL+2, 0-1, eleH+2])
    plt.xticks([])
    plt.yticks([])
    plt.axis('off')

    return plt.gcf()


def main():
    os.makedirs('./crack_svg', exist_ok=True)
    os.makedirs('./crack_jpeg', exist_ok=True)

    gif_frames = []

    for step in range(0, steps, dt):
        fig = plot_cracks(step)

        fig.savefig(f'./crack_jpeg/crack_{0.001 * step:.4f}ms.jpg', format='jpeg', dpi=600)

        img = fig2img(fig)  # Convert figure to image
        gif_frames.append(img)

        plt.close(fig)

    # Save GIF
    gif_frames[0].save('./crack_jpeg/5_25ms.gif', save_all=True, append_images=gif_frames[1:], duration=200, loop=0)


def fig2img(fig):
    """Convert a Matplotlib figure to a PIL Image and return it"""
    import io
    buf = io.BytesIO()
    fig.savefig(buf, format='png')
    buf.seek(0)
    img = Image.open(buf)
    return img


if __name__ == "__main__":
    main()
