...
smooth_l1_criterion = nn.SmoothL1Loss().to(device)
mse_criterion = nn.MSELoss().to(device)


# Define a combined loss function
def combined_loss(y_pred, y_true, alpha=0.5):
    smooth_l1_loss = smooth_l1_criterion(y_pred, y_true)
    mse_loss = mse_criterion(y_pred, y_true)
    return alpha * smooth_l1_loss + (1 - alpha) * mse_loss
...
# cscheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS, eta_min=1e-6)
...
for epoch in range(EPOCHS):
    ...
    for batch_param, batch_disp, batch_shear in train_loader_tqdm:
        ...
        loss = combined_loss(outputs, batch_shear, alpha=0.5)  # Adjust alpha as needed
...
new_input_displacement = denormalize2(new_input_displacement.cpu().numpy(), disp_scaler, scaling_strategy='symmetric_log', handle_small_values=True, small_value_threshold=1e-3, sequence=True)
real_shear = denormalize2(real_shear.cpu().numpy(), shear_scaler, scaling_strategy='symmetric_log', handle_small_values=True, small_value_threshold=1e-3, sequence=True)
predicted_shear = denormalize2(predicted_shear.cpu().numpy(), shear_scaler, scaling_strategy='symmetric_log', handle_small_values=True, small_value_threshold=1e-3, sequence=True)
