...
class TransformerAE_Model(nn.Module):
    def __init__(self, num_features_input_parameters, num_features_input_displacement, sequence_length,
        ...
        super(TransformerAE_Model, self).__init__()
...
class InformerAE_Model(nn.Module):
    def __init__(self, num_features_input_parameters, num_features_input_displacement, sequence_length,
...
# model = TransformerAE_Model(NUM_FEATURES_INPUT_PARAMETERS, NUM_FEATURES_INPUT_DISPLACEMENT, SEQUENCE_LENGTH).to(device)
# model = InformerAE_Model(NUM_FEATURES_INPUT_PARAMETERS, NUM_FEATURES_INPUT_DISPLACEMENT, SEQUENCE_LENGTH).to(device)
...
early_stopping = EarlyStopping(PATIENCE, verbose=True, path=f"checkpoints/{type(model).__name__}.pt")
...
for epoch in range(EPOCHS):
    ...
    print(f'Epoch [{epoch + 1}/{EPOCHS}], Train Loss: {epoch_train_loss:.6f}, Train R2: {epoch_train_r2:.4f}, Val Loss: {val_loss:.4f}, Val R2: {val_r2:.4f}')
